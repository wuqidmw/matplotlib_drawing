...
def get_boxplot(data: pandas.Series, tick_labels=None, x_label=None, y_label=None, title=None,
    ...
    for key, value in stats.items():
        if vert:
            plt.text(1.1, value, f'{key}: {value:.{dcm}f}', verticalalignment='center')
            plt.plot(1, value, 'ro', markersize=2)
        else:
            plt.text(value, 1.2, f'{key}: {value:.{dcm}f}', verticalalignment='center', rotation=90)
    ...
    # 例如，将图例放置在图表外部
    ax = plt.gca()
    ax.legend([f'MAX={data.max():.{dcm}f}\nMIN={data.min():.{dcm}f}\nMEAN={data.mean():.{dcm}f}'], bbox_to_anchor=(1, 1))
    # 对文件名中特殊字符进行去除,添加统一时间戳前缀,保存图形并显示
    plt.savefig(sanitize_and_check_filename(title) + '.png')
    plt.show()


def __find_outliers(data, threshold=1.5):
    """
    找出数据集中的离群值。
