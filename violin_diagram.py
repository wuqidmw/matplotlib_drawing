...
colors = global_con.color_scheme(0.8, 0.85)
...
def custom_violinplot(data, position=1, title=None, x_label=None, sub_title=None,
...
def custom_violinplots(data1, data2, position1=1, position2=2, title=None, x_labels=None, sub_titles=None,
    ...
    # 绘制小提琴图
    plot_violin(ax, data1, position1, colors[0])
    plot_violin(ax, data2, position2, colors[1])

    # 绘制均值和极值（使用boxplot）
    plot_boxplot(ax, [data1, data2], [position1, position2], show_means, show_extrema)

    # 绘制百分位数和均值
    plot_quantiles(ax, data1, position1)
    plot_quantiles(ax, data2, position2)

    # 添加统计标签
    add_stats_label(ax, data1, position1, colors[0])
    add_stats_label(ax, data2, position2, colors[1])
    ...
    if sub_titles:
        add_sub_titles(ax, data1, data2, position1, position2, sub_titles)

    plt.show()


def plot_violin(ax, data, position, color):
    parts = ax.violinplot([data], positions=[position], showmeans=False, showmedians=False, showextrema=False)
    for pc in parts['bodies']:
...
def plot_boxplot(ax, data, positions, show_means, show_extrema):
    ax.boxplot(data, positions=positions, widths=0.1, showfliers=False, showmeans=show_means,
               showcaps=show_extrema, whiskerprops={'linewidth': 2})


def plot_quantiles(ax, data, position):
    quantiles = [25, 50, 75, 90]
...
def add_stats_label(ax, data, position, color):
    stats = {
        'MAX': np.max(data),
        'MIN': np.min(data),
        'MEAN': np.mean(data)
...
def add_sub_titles(ax, data1, data2, position1, position2, sub_titles):
    for pos, sub_title, data in zip([position1, position2], sub_titles, [data1, data2]):
        data_info = f"数据量：{len(data)}"
        sub_title_full = f"车型：{sub_title}; {data_info}"
        q1, q3 = np.percentile(data, [25, 75])
        iqr = q3 - q1
        lower_bound = q1 - 1.5 * iqr
        upper_bound = q3 + 1.5 * iqr
